...
namespace RPGCore.Dialogue.Editor
{
    public class DialogueEditorWindow : EditorWindow
    {
        ...
        public static bool OpenWindowWithAsset(int instanceId, int line)
        {
            ...
            if (openAsset is DialogueGroupDataSO)
            {
                ...
                else
                {
                    ...
                    windowInstance.titleContent = new GUIContent("Dialogue Editor Window",
                        Resources.Load<Texture>("DialogueGroupIcon"));
        ...
        {
            windowInstance = GetWindow<DialogueEditorWindow>();
            windowInstance.titleContent =
                new GUIContent("Dialogue Editor Window", Resources.Load<Texture>("DialogueGroupIcon"));
            windowInstance.minSize = new Vector2(800, 600);
        }
        ...
        private void OnEnable()
        {
            ...
            groupItemHierarchyListView.bindItem = (item, index) =>
            {
                ...
                {
                    (item as EditorHierarchyItem).ItemNameLabel.text =
                        currentOpenedGroupData.dialogueItems[index].name.Split("-").Last();
                    (item as EditorHierarchyItem).name = (item as EditorHierarchyItem).ItemNameLabel.text;
                    (item as EditorHierarchyItem).DeleteItemBtn.RegisterCallback<ClickEvent>(ce =>
                    {
                        DeleteItem(currentOpenedGroupData.dialogueItems[index]);
                    });
                }
            };
            ...
            //设置控件回调函数
            saveBtn.RegisterCallback<ClickEvent>(ce =>
            {
        ...
        private void UpdateHierarchyView()
        {
            ...
            int selectedIndex =
                currentOpenedGroupData.dialogueItems.IndexOf(currentOpenedGroupData.GetOpenedEditorItem());
            ...
            dialogueWindow.OpenDialogue(
                ...
                ok =>
                {
                    ...
                    if (string.IsNullOrWhiteSpace(name) ||
                        DialogueEditorUtility.DialogueItemExists(currentOpenedGroupData, name))
                    {
                        dialogueWindow.CloseDialogue();
                        dialogueWindow.OpenDialogue(
                            "Create Item Failed",
                    ...
                    {
                        DialogueItemDataSO itemData =
                            DialogueEditorUtility.CreateDialogueItemData(currentOpenedGroupData, name, description);
                        ;
        ...
        public void DeleteItem(DialogueItemDataSO itemToDelete)
        {
            ...
            dialogueWindow.OpenDialogue(
                ...
                ok =>
                {
                    ...
                    if (deletedIsCurrent)
                    {
                        currentOpenedGroupData.SetOpenedEditorItem(currentOpenedGroupData.dialogueItems.FirstOrDefault()
                            .name);
                        //这里不执行保存操作
        ...
        {
            string openedPath = EditorUtility.OpenFilePanelWithFilters("Select Group", "",
                new string[] { "DialogueGroupDataSO", "asset" });
        ...
        public void OnDialogueItemSelectionChange(IEnumerable<object> selectedItems)
        {
            ...
            if (itemData != null && itemData.name != previousItem.name)
            {
                ...
                //更新label名称显示
                editInfomationLabel.text =
                    $"[{currentOpenedGroupData.name}] -> [{currentOpenedGroupData.GetOpenedEditorItem().name.Split("-").Last()}]";
            }
        }
